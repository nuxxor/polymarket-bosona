...
def main():
    ...
    with patch.object(ex.engine.old,'context',side_effect=context):
        good=r.candidate(client,venues[250],market,{}, {},250)
    ...
    wide=copy.deepcopy(venues[250])
    wide[start+30250][0]=book(start+30250,'.4','.45')
    legacy=ex.run(client,wide,[trade(30500)],market)
    corrected=ex.run(client,wide,[trade(30500)],market,separate_arrival=True)
    assert legacy['unknown']==['unobserved_acceptance_book']
    assert not corrected['unknown'] and corrected['actual']['q'][0]==5
    assert ex.engine.quote(ex.book_tuple(wide[start+30250][0]),start+30250) is None
    assert not ex.arrival_valid(book(start-5000),start)
    assert not ex.arrival_valid(book(start,'.45','.4'),start)
    checks.append('arrival spread widening does not erase an observed book; decision spread, stale and crossed checks remain')
    wide_p0=copy.deepcopy(venues[250])
    wide_p0[start+180250][0]=book(start+180250,'.4','.45')
    with patch.object(ex.engine.old,'context',side_effect=context),patch.object(r.prior.ex.engine.old,'context',side_effect=context):
        before=r.prior.candidate(client,wide_p0,market,{}, {},250)
        after=r.candidate(client,wide_p0,market,{}, {},250)
    assert before['gaps'].get('missing_arrival_book') and not after['gaps']
    assert after['state']['entered'] and before['trace'][0]['intent']==after['trace'][0]['intent']
    assert before['trace'][0]['sent_cash_limit']==after['trace'][0]['sent_cash_limit']
    checks.append('P0 also separates arrival validity from decision spread without changing intent or sent price cap')
    ...
    for x in rows:
        ...
        prior=r.a.read(r.HERE/'legacy_arrival_results/markets'/f'{x["start"]}.json')
        assert full['control']==prior['control']
        for key,value in prior['arms'].items():
            if not value['unknown']:
                assert full['arms'][key]==value
    ...
    checks.append('arrival fix leaves all previously defined real paths and P0 controls identical; only false-null paths change')
