"""Isolated timing scenarios; frozen decisions and cash-reserve rule retained."""
...
def run(client_books, exchange_books, prints, market, mode='queue_back', offset=D(0), hedge=False, delay=250, learn_extra=0, client_ticks=None, exchange_ticks=None, fund_hedge=False, cancel_delay=None, trade_shift=0, tie_order='unknown', trace=False, separate_arrival=False):
    ...
    assert cancel_delay in (250,750) and trade_shift in (0,-500)
    ...
    def cancel(o, now):
        if not o['cancel_requested']:
            ...
            push(now+cancel_delay,0,'cancel',o['id'])
