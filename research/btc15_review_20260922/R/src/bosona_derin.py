...
try:
    ...
    import json as orjson  # Public-data shadow also runs with only the stdlib.
...
def policy():
    ...
    for S in sorted(mm):
        ...
        for age in (210, 240, 270, 280, 290):
            ...
            policies = dict(favorite=fav, twap_value=None, twap_drift_value=None, twap_fitted_value=None, twap_empirical_value=None,
                            spot_value=None, rebound=None, rebound_divergence=None, minute_turn=None, pressure_turn=None)
            details = {}
            ...
            cheap = 0 if mids[0] < mids[1] else 1
            sign = 1 if cheap == 0 else -1
            policies['rebound'] = rebound_side(f, pair)
            rebound = policies['rebound'] is not None
            ...
            minute_turn = sign*f['spot_z'] < 0 and sign*f['return1'] > 0 and pair[cheap][1] < .5
            own_flow = f['taker_buy_fraction'] if cheap == 0 else 1-f['taker_buy_fraction']
            policies['minute_turn'] = cheap if minute_turn else None
            policies['pressure_turn'] = cheap if minute_turn and own_flow > .6 else None
    ...
    for age in (210, 240, 270, 280, 290):
        for rule in ('favorite', 'twap_value', 'twap_drift_value', 'twap_fitted_value', 'twap_empirical_value', 'spot_value',
                     'rebound', 'rebound_divergence', 'minute_turn', 'pressure_turn'):
