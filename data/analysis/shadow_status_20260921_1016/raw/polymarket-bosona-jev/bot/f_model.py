...
def read_start_report(db, S, now_ms):
    with sqlite3.connect(Path(db).resolve().as_uri()+'?mode=ro',uri=True,timeout=.25) as con:
...
def features(streams, reference, S, now_ms, max_elapsed=200):
    """streams: kaynak -> (sirali received_ms listesi, (obs_ms, fiyat) listesi)."""
    if not math.isfinite(reference) or reference <= 0 or not 3 <= now_ms / 1000 - S <= max_elapsed < 300:
    ...
    return x, dict(spot=spot, twap60=twap, sigma=sigma, spot_received_ms=rcv,
                   ...
                   trajectory={s:[dict(seconds_ago=seconds, price=at(s,now_ms-seconds*1000)[0])
                                  for seconds in (60,30,10,0)] for s in ('spot','twap60')})
