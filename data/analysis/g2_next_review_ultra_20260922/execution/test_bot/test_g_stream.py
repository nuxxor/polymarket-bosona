...
def scenario(path, kind):
    ...
    canceled = []
    window = dict(emir=[], klip=5, cozuldu=False, kol='G')
    if kind == 'unreconciled_existing':
        b.LIVE = True  # SDK initialization remains bypassed; all order methods mocked.
        row = dict(oi=0, p=.54, boy=5, pay=0, durum='acik', oid='test', dolumlar=[])
        window['emir'].append(row)
        b.kapat = lambda *args: canceled.append(True)
    ...
    if kind in ('burst', 'unreconciled_existing'):
    ...
    with patch('websockets.sync.client.connect', lambda *a, **kw: Book()), \
        ...
        b.taze_izle(('btc', int(now)-10), window, {0:'u', 1:'d'})
    for thread in threads:
        thread.join(2)
        assert not thread.is_alive()
    assert not errors, errors
    expected = (None, None) if kind in ('delta_before_snapshot', 'old') else (.55, .56)
    ...
    assert 'PING' in sent
    if kind == 'unreconciled_existing':
        assert canceled, 'Unreconciled account retained a resting G order'


if __name__ == '__main__':
    path = Path(sys.argv[1]) if len(sys.argv)>1 else Path(__file__).with_name('ab.py')
    for kind in ('burst', 'mixed', 'old', 'delta_before_snapshot', 'reconnect', 'unreconciled_existing'):
        scenario(path, kind)
