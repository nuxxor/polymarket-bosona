"""M4: varsayilan salt-okunur kontrol; --live yalniz operator baslatmasi."""
import argparse
import contextlib
import fcntl
import hashlib
...
def verify(root):
    manifest = json.loads((root/'M4_RELEASE.json').read_text())
...
def preflight(root, protocol):
    ...
    spec = importlib.util.spec_from_file_location('m4_checked_bot', root/'ab.py')
    b = importlib.util.module_from_spec(spec)
    saved = sys.argv
    try:
        sys.argv = [str(root/'ab.py'), '--lane-f']
    ...
    try:
        ...
        prices = json.loads((root/'../data/d_olcum/latest.json').read_text())
        now = time.time()*1000
        ages = {s: {k: now-prices['prices'][s][k] for k in ('received_ms', 'observed_ms')}
                for s in ('crypto_prices_chainlink', 'crypto_prices_twap_sixty')}
        if not prices.get('connected') or any(not 0 <= age <= 3000 for v in ages.values() for age in v.values()):
            raise CheckFailed('Fiyat kaydedicisi eski')
        if g.errors:
...
def activate_files(root, state, proof, now):
    ...
    if any((root/n).exists() for n in ('RUN_M4.json', 'STATE_f.json', 'BUDGET_F.json', 'LOG_f.jsonl')):
        raise CheckFailed('Pilot daha once hazirlanmis; butce/sure sifirlanamaz')
    if not (root/'STOP_F').exists():
    ...
    with (root/'RUN_M4.json').open('x') as f:
        json.dump(dict(proof=proof, budget=budget), f, indent=2)
    state = {**state, 'st': {**state['st'], 'f_budget_id': budget['id'], 'durdu': None}}
    for name, data in [('BUDGET_F.json', budget), ('STATE_f.json', state)]:
        with (root/name).open('x') as f:
            json.dump(data, f)
    (root/'STOP_F').rename(root/'STOP_F.operator_once')
    return budget
...
def main():
    ...
    with (root/'.m4_operator.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        protocol = verify(root)
        if (root/'RUN_M4.json').exists():
        ...
        if args.live:
            ...
            os.execv(sys.executable, [sys.executable, '-u', str(root/'f.py'), '--live', '--dk', '30'])
        print('M4 PARKTA: kontrol gecti; emir gonderilmedi, yeni butce acilmadi.')
