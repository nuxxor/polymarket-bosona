...
def load_prices():
    ...
    excluded = []
    for p in sorted((ROOT/'data/tape_cl_direct').glob('cld_202609*.gz')):
        try:
...
def summary(rows, field='marginal_cash_pnl'):
    ...
    return dict(records=len(rows), windows=len(values), shares=sum(r.get('qty', 0) for r in rows),
                pnl=total, positive_windows=sum(v > 0 for v in values), days=dict(days),
...
def snapshots(con, m, times):
    ...
    if any(row is None for row in latest_full):
        return dict.fromkeys(times)
    begin = min(row[0] for row in latest_full)
...
if __name__ == '__main__':
    ...
    parser.add_argument('action', choices=['fetch', 'check', 'analyze', 'watch'])
