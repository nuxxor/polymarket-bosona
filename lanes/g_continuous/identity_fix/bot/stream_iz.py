...
def roster(seconds):
    ...
    with requests.Session() as http:
        for start in range(now//300*300-300,(now+seconds)//300*300+301,300):
            ...
            response = http.get('https://gamma-api.polymarket.com/markets/slug/'+slug,timeout=10)
...
def capture(output, markets, auth, seconds, account_count, stop, connector=None, queue_size=4096, compressed=False, health_path=None, stop_path=None):
    from websockets.sync.client import connect
    connector = connector or connect
    output.mkdir(parents=True,exist_ok=True)
    session = uuid.uuid4().hex
    path = output/(f'{session}.jsonl.gz' if compressed else f'{session}.jsonl')
    pending = queue.Queue(maxsize=queue_size)
    stats = {k:Counter() for k in ('market','user')}
    pongs = {'market':0,'user':0}
    def health(status):
        if health_path is None: return
        data = dict(pid=os.getpid(),at_ns=time.time_ns(),status=status,pongs=pongs,
                    rejected=sum(x['rejected'] for x in stats.values()),
                    queue_overflow=sum(x['queue_overflow'] for x in stats.values()),
                    file=str(path),capture_dir=str(output))
                        ...
                        for index,message in enumerate(messages):
                            try:
                                ...
                                reason = str(error) if str(error) in (
                                    'numeric field','identity field','enum field','incomplete order',
                                    'incomplete trade','market outside roster','asset outside market') else type(error).__name__
                                emit(channel,'rejected',received,connection=connection,frame=stats[channel]['frames'],index=index,reason=reason)
                                continue
    ...
    try:
        with (gzip.open(path,'xt',encoding='utf8',compresslevel=1) if compressed else path.open('x',encoding='utf8')) as file:
            ...
            while any(t.is_alive() for t in threads) or not pending.empty():
                if (output/'STOP_M7').exists() or (stop_path and stop_path.exists()) or time.monotonic()>=deadline:
                    stop.set()
                try:
                ...
                if time.monotonic()-flushed>=1:
                    file.flush()
                    if compressed and shutil.disk_usage(output).free < 2*1024**3:
                        raise OSError('recording disk reserve')
                    health('active')
                    flushed = time.monotonic()
        ...
        for thread in threads:
            if thread.ident is not None:
                thread.join(12)
        try: health('failed' if writer_failed else 'ended')
        except OSError: pass
