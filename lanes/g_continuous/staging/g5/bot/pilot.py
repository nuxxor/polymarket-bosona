...
def verify(root):
    ...
    if (protocol['minutes'], protocol['loss_limit'], protocol['clip'], protocol.get('continuous')) != (None, 100 if protocol.get('experiment') in ('G4','G5') else 10, 5, True):
...
def activate_files(root, state, proof, now):
    ...
    if (root/'protocol.json').exists() and json.loads((root/'protocol.json').read_text()).get('experiment') in ('G4','G5'):
        raise CheckFailed('G experiment activation requires its one-time operator transition')
