"""G5 quote-retention experiment; no inferred Bosona alpha signal."""
...
def arm(start):
    """Balanced adjacent UTC windows; fixed before fills, stable across restarts."""
    flip = sha256(f'G5-retain-v1:{int(start)//600}'.encode()).digest()[0] & 1
    return 'G5' if (int(start)//300 % 2) ^ flip else 'G4_FIXED'


def retain(price, bid, own, opposite, label):
    # Caller must first pass size, risk, freshness, tick and time checks.
    return price <= bid+1e-9 and (price >= bid-.02-1e-9 or
                                (label == 'G5' and own >= opposite-1e-6))
