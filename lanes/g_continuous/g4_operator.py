...
def package():
    protocol = json.loads((HERE/'G4_PROTOCOL.json').read_text())
    assert protocol['loss_limit'] == LIMIT
    assert digest(HERE/'G4_package.tar.gz') == protocol['package_sha256']
    with tarfile.open(HERE/'G4_package.tar.gz') as archive:
...
def activate(root, files, old, new, state, proof, now):
    ...
    assert not (root/'G4_ACTIVATION.json').exists(), 'activation already attempted'
    ...
    budget = dict(experiment='G4', limit=LIMIT, anchor=anchor, cutoff=anchor-LIMIT,
                  end_ms=None, continuous=True, created_ms=now, previous_stop=st.get('durdu'))
    budget['id'] = budget_id(budget)
    archive = root/'G4_before_activation'
    archive.mkdir()
    ...
    with (root/'G4_ACTIVATION.json').open('x') as file:
...
def main():
    ...
    with (root/'.g_operator.lock').open('a') as lock:
        ...
        if installed == new:
            ...
            marker = json.loads((root/'G4_ACTIVATION.json').read_text())
            assert marker['budget'] == budget and budget['experiment'] == 'G4' and budget['limit'] == LIMIT
            if writers:
                print('G4 zaten LIVE; butce/sure sifirlanmadi.', flush=True)
                return
        else:
            assert not (root/'G4_ACTIVATION.json').exists(), 'incomplete activation; remains stopped'
            (root/'STOP_G').touch()
            ...
            for _ in range(60):
                ...
                try:
                    ...
                    print('G4 hesap teyidi bekleniyor: '+type(error).__name__, flush=True)
                    time.sleep(15)
            ...
            print(json.dumps(dict(event='G4_FRESH_BUDGET', **budget)), flush=True)
