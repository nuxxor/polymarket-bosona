...
def apply(root,old,new):
    ...
    if current==old:
        archive=root/('transport_fix_backup_'+str(time.time_ns()))
        archive.mkdir()
        ...
        for name in ('ab.py','G_RELEASE.json'):
            temporary=root/(name+'.transport_tmp')
...
def main():
    ...
    print('API bekleme yamasi dogrulandi; ayni butceyle operator LIVE devam yolu.',flush=True)
...
if __name__=='__main__':
    try:main()
        ...
        print('TRANSPORT_FIX_FAILED: '+type(error).__name__,flush=True)
