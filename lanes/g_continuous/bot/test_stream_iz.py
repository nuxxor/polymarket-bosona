...
def main():
    # Direct lookup must retain the preceding window and reject a mismatched response.
    with patch('requests.Session') as session,patch.object(s.time,'time',return_value=700):
        http = session.return_value.__enter__.return_value
        def response(url,timeout):
            slug = url.rsplit('/',1)[1]
            assert url.startswith('https://gamma-api.polymarket.com/markets/slug/')
            return SimpleNamespace(raise_for_status=lambda:None,json=lambda:dict(
                slug=slug,conditionId=CIDS[0],outcomes='["Up","Down"]',clobTokenIds='["10","11"]'))
        http.get.side_effect = response
        assert [m['S'] for m in s.roster(120)]==[300,600,900]
        http.get.side_effect = lambda *a,**k:SimpleNamespace(raise_for_status=lambda:None,
            json=lambda:dict(slug='wrong-market'))
        try:
            s.roster(120)
        except AssertionError:
            pass
        else:
            raise AssertionError('Wrong market accepted')
